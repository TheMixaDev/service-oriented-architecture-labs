  ...
  protected ResponseEntity<Object> handleHttpMessageNotReadable(
    ...
    if (cause instanceof InvalidFormatException) {
      ...
      if (ife.getTargetType().isEnum()) {
        message =
            "Некорректные данные в теле запроса. Допустимые значения поля priority: LOW, MEDIUM,"
                + " HIGH.";
      } else if (expectedType.equals("double") || expectedType.equals("integer")) {
        message =
            String.format(
                "Некорректные данные в теле запроса. Тип поля %s должен быть %s.",
    ...
    String message =
        ex.getBindingResult().getFieldErrors().stream()
            .map(
                error -> {
                  String fieldPath =
                      error.getField().replace("fromLocation", "from").replace("toLocation", "to");
